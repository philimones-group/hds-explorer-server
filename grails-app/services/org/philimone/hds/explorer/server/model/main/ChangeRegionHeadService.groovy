...
class ChangeRegionHeadService {
    ...
    ArrayList<RawMessage> validate(RawChangeRegionHead changeHead) {
        ...
        if (!isBlankEventDate && newHeadExists && changeHead.eventDate < newHead.dob){
            errors << errorMessageService.getRawMessage(RawEntity.CHANGE_HEAD_OF_REGION, "validation.field.dob.not.greater.date", ["changeHead.eventDate", StringUtil.format(newHead.dob)], ["eventDate","member.dob"])
        }
        //C6. Check Age of the new head of Household
        if (newHeadExists && GeneralUtil.getAge(newHead.dob) < Codes.MIN_HEAD_AGE_VALUE){
            errors << errorMessageService.getRawMessage(RawEntity.CHANGE_HEAD_OF_REGION, "validation.field.changeregionhead.dob.head.minage.error", [StringUtil.format(newHead.dob), Codes.MIN_HEAD_AGE_VALUE+""], ["member.dob"])
        }
