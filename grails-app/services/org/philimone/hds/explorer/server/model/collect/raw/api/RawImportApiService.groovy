...
class RawImportApiService implements DataBinder {
    ...
    RawParseResult<RawHouseholdProxyHead> parseChangeProxyHead(NodeChild xmlNode) {
        ...
        if (!rootnode.equalsIgnoreCase("RawHouseholdProxyHead")) {
            errors << errorMessageService.getRawMessage("validation.field.raw.parsing.rootnode.invalid.error", [rootnode])
            return new RawParseResult<RawHouseholdProxyHead>(null, errors)
        }
        ...
        def rawInstance = new RawHouseholdProxyHead(params)
        rawInstance.id = params.id

        return new RawParseResult<RawHouseholdProxyHead>(rawInstance, errors)
